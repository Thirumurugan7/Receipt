...
**The Bazantic recipe is not published.** The gateway manifest and recipe are written and pass every rule `baz recipe --help` documents, and the gateway itself is configured and connection-tested against a tunnelled facilitator — but nobody has pressed Activate, so the 26-character `gateway_slug` the platform assigns does not exist yet and the recipe still binds to placeholders. Those placeholders are deliberately obvious (`REPLACE_WITH_PAYOUT_ACCOUNT_UUID`, slugs containing `replaceme`) rather than plausible fakes, and a test asserts they stay that way. Gateway creation is self-serve behind `baz login`; an earlier draft of this file claimed it required a two-day manual review, which was wrong — that review is for commercial provider listings, not hackathon gateways.
